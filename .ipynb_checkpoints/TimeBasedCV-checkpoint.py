...
class TimeBasedCV(object):
    ...
    def split(self, data, first_split_date, second_split_date, date_column='eom', gap=0):
        '''
        ...
        # Pre-convert date column to date objects once for efficiency
        # This avoids calling .dt.date multiple times in the loop (which is very slow)
        date_series = data[date_column].dt.date
        ...
        while end_test < max_date:
            # train indices:
            cur_train_indices = list(data[(date_series>=start_train) & 
                                     (date_series<end_train)].index)
            # validation indices:
            cur_val_indices = list(data[(date_series>=start_val) & 
                                     (date_series<end_val)].index)

            # test indices:
            cur_test_indices = list(data[(date_series>=start_test) &
                                    (date_series<end_test)].index)
